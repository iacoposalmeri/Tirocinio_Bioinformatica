...
for cutoff in cutoffs:
    ...
    for ks in k_skb:    
        ...
        # Trasformiamo di nuovo in DataFrame per comodità (opzionale ma consigliato)
        X_train_scaled = pd.DataFrame(X_train_scaled, columns=X_train_skb.columns, index=X_train_skb.index)
        X_test_scaled = pd.DataFrame(X_test_scaled, columns=X_test_skb.columns, index=X_test_skb.index)
...
df_risultati_cv.to_csv("risultati_RCLR_crossvalidation_skb.csv", index=False)